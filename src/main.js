...
// Se le envía un parámetro Data, lo que permite decidir que se quiere mostrar o pedir.
  ...
  // Se crea un for con la variable harryPotter el cual recorre el valor Data
    ...
    characters = `<img class='shield' src='${data[harryPotter].shield}'>
...
document.getElementById('selectbox').onchange = function () { // Llamando al id selectBox y ejecuta la función
  ...
  const filterResult = filterData(selection);
...
document.getElementById('houseFilterSelect').onchange = function () {
  ...
  const houseFilterData = orderHouse(selection);
...
document.getElementById('wandSelect').onchange = function () {
  ...
  const wandsFilterData = orderWand(selection);
